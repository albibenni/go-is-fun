...
func chargeForLineItem[T lineItem](newItem T, oldItems []T, balance float64) ([]T, float64, error) {
	var zVal []T
	price := newItem.GetCost()
	if price > balance {
		return zVal, 0.0, errors.New("insufficient funds")
	}
	newOldItems := append(oldItems, newItem)
	currBalance := balance - price
	return newOldItems, currBalance, nil
}
